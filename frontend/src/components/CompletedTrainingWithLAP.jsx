...
export default function CompletedTrainingWithLAP() {
  ...
  const lapSubmittedTrainingIds = new Set([
    ...userLaps.map(lap => Number(lap.lap_training)),
  ]);

  const isLapSubmitted = (trainingId) => {
    const trainingIdNum = Number(trainingId);
    return lapSubmittedTrainingIds.has(trainingIdNum);
  };
  ...
  const handleLAPSubmitSuccess = (newLap) => {
    
    setUserLaps(prev => [...prev, newLap]);
  ...
  return (
    <main className="flex-1 overflow-auto p-1">
                  ...
                  <tr key={enrolled.id} className="border border-gray-300">
                    ...
                    <td className="border border-gray-300 px-4 py-2 text-center">
                      <button
                        ...
                        className={`text-white font-semibold py-1 px-3 rounded inline-flex items-center ${
                          lapSubmitted
      ...
      {showLAPModal && selectedTraining && (
        <LAP
          training={selectedTraining}
          onClose={() => setShowLAPModal(false)}
          onSubmitSuccess={handleLAPSubmitSuccess}
        />
      )}
    </main>
  );
